...
def within_alignment_blast_check(sv_id,ref_chr,ref_sta,ref_end,qry_chr,qry_sta,qry_end,anchor_info,anchor_str):
    ...
    if ref_sta > gap_flank:
        ...
        left_seq = re.sub('.*N', '', "".join(subprocess.check_output("samtools faidx " + ref_genome + " " +  left_region, shell=True).decode().split("\n")[1:-1]))
    ...
    right_seq = re.sub('N.*', '', "".join(subprocess.check_output("samtools faidx " + ref_genome + " " +  right_region, shell=True).decode().split("\n")[1:-1]))
