...
with open(prefix + ".sam_1-6_col", "w") as sam_infor_file, open(prefix + ".sam_1-6_col.unique", "w") as unique_sam, open(prefix + ".anchor.tsv", "w") as raw_alignment_tsv, open(prefix + ".variants_within_alignments.bed", "w") as sv_bed_file:
    out_list = ["ref_start","ref_end","query_start","query_end","ref_length","query_length","ref","query","tag"]
    raw_alignment_tsv.write("\t".join(out_list) + "\n")
    ...
    for line in forward_alignments:
        line = line.strip()
        if line != "":
            sam_infor_file.write(line + "\tForward\n")
            cells = line.split("\t")
            anchor_infor = get_forward_anchor(cells)
            if "\t".join(map(str,anchor_infor)) in uniqe_anchor_set:
                unique_sam.write(line + "\n")
                raw_alignment_tsv.write("\t".join(map(str,anchor_infor)) + "\tForward_Unique\n")
                Uniq_For += 1
                sv_order = SV_calling(cells,sv_order,anchor_infor,"Forward",sv_bed_file)
            else:
                raw_alignment_tsv.write("\t".join(map(str,anchor_infor)) + "\tForward_Repetitive\n")
    ...
    for line in reverse_alignments:
        line = line.strip()
        if line != "":
            sam_infor_file.write(line + "\tReverse\n")
            cells = line.split("\t")
            anchor_infor = get_reverse_anchor(cells)
            if "\t".join(map(str,anchor_infor)) in uniqe_anchor_set:
                unique_sam.write(line + "\n")
                raw_alignment_tsv.write("\t".join(map(str,anchor_infor)) + "\tReverse_Unique\n")
                Uniq_Rev += 1
                sv_order = SV_calling(cells,sv_order,anchor_infor,"Reverse",sv_bed_file)
            else:
                raw_alignment_tsv.write("\t".join(map(str,anchor_infor)) + "\tReverse_Repetitive\n")
